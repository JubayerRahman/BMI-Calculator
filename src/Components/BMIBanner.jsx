...
const BMIBanner = () => {
  ...
  const idealWeight = (height) => {
    let minWeight = Math.round(18.5 * (parseFloat(height) / 100) **2).toFixed(1)
    let maxWeight = Math.round(24.9 * (parseFloat(height) / 100) **2).toFixed(1)
            ...
            <form className='text-left'>
               <div className='flex flex-col md:flex-row gap-2 justify-between'>
                    <div className='lg:w-[45%]'>
                            ...
                            <h1 className='text-primary text-2xl font-bold'>cm</h1>
                            <input onChange={(e)=>setHeight(e.target.value)} className='p-[5px] w-full outline-none bmiNumber border-none text-primary text-2xl font-[500]' maxLength="3" type="text" placeholder="0" id="Height" />
                            </div>
                    </div>
                    <div className='lg:w-[45%]'>
                            ...
                            <br/>
                            <div className='border-2 w-[100%] border-primary rounded-md flex flex-row-reverse p-[5px] items-center justify-around'>
                            <h1 className='text-primary text-2xl font-bold'>Kg</h1>
                            <input onChange={(e)=>setweight(e.target.value)} className='p-[5px] w-full outline-none bmiNumber border-none text-primary text-2xl font-[500]' maxLength="3" type="text" placeholder="0" id="Weight" />
                            </div>
                    </div>
               </div>
            </form>
            <div className='bg-primary rounded-xl md:rounded-tr-[100px] md:rounded-br-[100px] mt-[20px] p-[20px] text-white text-left'>
                ...
                <div className={BMIresult <= 0? "hidden":"flex flex-col md:flex-row gap-3 items-center justify-between"}>
                  <h1 className='text-xl bmiNumber flex flex-col'>Your BMI is 
                    <span className='text-5xl font-bold'> {BMIresult}</span>
                  </h1>
                  <p className='md:w-[55%]'>
                  Your BMI suggests you're {BMImeaning}.
